...
- **Paroles synchronisées avec Spotify** — quand LRCLIB fournit des paroles horodatées (LRC), la fiche du disque les **fait défiler en surbrillance au rythme de la lecture Spotify** : le morceau en cours s'ouvre tout seul et la ligne chantée est mise en avant (horloge locale interpolée entre deux relevés du « en cours d'écoute »).
...
- **Statistiques** — un tableau de bord chiffré de la collection : nombre de disques / artistes / labels, part enrichie, avec paroles, notés, en concert, masqués, **valeur estimée de la collection** (somme du prix le plus bas du marché Discogs) et **disques les plus cotés**, et des classements (par décennie, genres, **artistes les plus présents**, labels, pays de pressage, formats, répartition des notes) — chaque barre renvoie vers la recherche filtrée correspondante.
- **Cote & valeur (Discogs)** — sans appel API supplémentaire (la donnée arrive avec l'enrichissement) : la fiche d'un disque affiche son **prix le plus bas du marché** et sa cote communautaire (possédé / recherché), et les statistiques additionnent la valeur de toute la collection.
...
- **Recherche croisée** — filtrez par artiste, instrument (« qui joue de la basse »), genre, style, label, pays, **format (33/45 tours, LP/EP/Single…)**, décennie, **note minimale (★+)**, version, tag, emplacement, et par **données manquantes** (sans année, sans pochette, sans paroles, sans crédits… cumulables — pratique pour repérer ce qui reste à compléter). Tous les filtres et la page courante vivent dans l'URL : le bouton retour revient exactement où vous étiez.
- **Recherche dans les paroles** — un onglet « Paroles » dans la recherche : tapez quelques mots d'une chanson et retrouvez le disque, avec l'extrait surligné et le morceau concerné (recherche plein texte PostgreSQL sur les paroles Genius / LRCLIB).
...
- **Détection de doublons** — une page qui regroupe les disques correspondant à une même œuvre (même *master* Discogs, ou même artiste + titre normalisés) : repérez les multiples pressages d'un album et masquez d'un clic ceux que vous ne voulez pas voir, sans les supprimer.
- **Opérations en masse** — dans la vue « Mur », un mode **Sélectionner** permet de cocher plusieurs disques et de les **masquer / afficher** ou de leur **ajouter / retirer un tag** d'un seul coup.
...
- **Rangement physique en 3D** — recréez votre pièce et vos meubles en 3D : **cubes (Kallax)**, cube unique, **tour à vinyles**, **bac à disques**, **vitrine**, **chevalet**, **étagère** et **cadre**. Déplacez-les **sur une grille** (avec collisions : ils ne se traversent plus, se collent bord à bord et restent dans la pièce), redimensionnez-les, réglez leur **position X/Y/Z** (pour empiler ou **accrocher au mur**), **verrouillez** ceux à ne plus bouger, et **cliquez une case** pour y ranger des disques (avec **réordonnancement** des disques par case). Les **pochettes apparaissent dans les meubles** : de tranche dans les cubes / étagères / vitrines (elles rétrécissent pour tenir dans la largeur), face avant dans les bacs / cadres / chevalets. **Localisez un disque** : depuis sa fiche, un clic sur l'emplacement ouvre la pièce, cadre la caméra et **fait sortir la pochette du meuble**, avec son emplacement précis (meuble · bac/casier/étagère · n°). Les anciens emplacements « texte » restent disponibles, et la recherche filtre toujours par emplacement.
- **Ajout manuel** — pour les disques absents de Discogs.
- **Page Paramètres organisée** — regroupée par catégories (Compte, Affichage, Collection, Connexions & API, Système, Zone de danger), avec une **barre de recherche** qui filtre les réglages en direct et un accès rapide par catégorie.
- **Réinitialiser la collection** — un bouton (admin, dans la « Zone de danger ») efface **tous les disques** de Vinylarium et leurs données dérivées (artistes, labels, genres, tags, paroles, pochettes), via une **fenêtre de confirmation** où il faut taper `RESET`. **Conserve** les meubles de rangement 3D, les profils et la configuration ; n'affecte pas le compte Discogs.
...
- **Langue de l'interface (par profil)** — Paramètres → Affichage : bascule **Français / English**. L'infrastructure d'internationalisation est en place (la navigation et les libellés courants sont traduits) ; le reste de l'interface se traduit progressivement et reste en français par défaut.
- **Affichage configurable (par profil)** — dans Paramètres, des interrupteurs activer/désactiver pour la **carte**, la **frise**, les **statistiques**, le **rangement**, les trois vues de bibliothèque (**mur / bac / pile**) et le bouton **« au hasard »** : ce qui est désactivé disparaît de la navigation et des sélecteurs (au moins une vue reste toujours active).
...
- **Partage public (lecture seule)** — un administrateur génère depuis les paramètres un **lien secret** (`/share/<token>`) qui ouvre une vue **lecture seule** de la collection, sans compte : pochettes, fiches, tracklists, crédits et paroles — **jamais** vos notes, votre rangement ni vos données perso. Le lien se régénère ou se désactive en un clic.
...
| `DISCOGS_CURRENCY` | Devise des prix du marché / de la valeur estimée de la collection (défaut `EUR`) |
...
- [x] **Localiser un disque dans la pièce 3D** : depuis la fiche, un clic sur l'emplacement cadre la caméra sur la case et **fait sortir la pochette du meuble** (avec l'emplacement précis affiché)
- [x] **Meubles verrouillables**, **réordonnancement des disques** dans une case, **emplacement précis** (meuble · bac/casier/étagère · n°) sur la fiche du disque, et **panneau d'édition repensé**
- [x] **Sauvegarde du rangement 3D** : la pièce, les meubles et le contenu des cases sont inclus dans l'export/restauration JSON
...
- [x] **Page Paramètres repensée** (catégories + barre de recherche) et **réinitialisation de la collection** (admin, confirmation par saisie de `RESET`, conserve les meubles de rangement)
- [x] **Internationalisation (FR/EN)** : infrastructure i18n par profil (sans dépendance), navigation + libellés courants traduits, migration progressive du reste
- [x] **Partage public lecture seule** : lien secret (token) ouvrant une vue read-only de la collection sans compte, généré/révoqué par un admin ; les données perso (notes, rangement) sont exclues
